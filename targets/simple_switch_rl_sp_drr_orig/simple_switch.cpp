...
SimpleSwitch::~SimpleSwitch() {
  ...
  for (size_t i = 0; i < nb_egress_threads; i++) {
#ifdef SSWITCH_PRIORITY_QUEUEING_ON
    egress_buffers.push_front(i, 0, nullptr, 0);
#else
...
int
SimpleSwitch::set_egress_priority_queue_rate(int port, size_t priority,  const uint64_t rate_Bps) {
  egress_buffers.set_rate(port, priority, rate_Bps);
  return 0;
}

int
SimpleSwitch::set_egress_queue_rate(size_t port, const uint64_t rate_Bps) {
  egress_buffers.set_rate(port, rate_Bps);
  return 0;
}

int
SimpleSwitch::set_all_egress_queue_rates(const uint64_t rate_Bps) {
  for (uint32_t i = 0; i < max_port; i++) {
    set_egress_queue_rate(i, rate_Bps);
  }
  return 0;
}

int
SimpleSwitch::set_egress_priority_queue_quantum(int port, size_t priority,  const uint64_t quantum) {
  egress_buffers.set_quantum(port, priority, quantum);
  return 0;
}
...
SimpleSwitch::enqueue(port_t egress_port, std::unique_ptr<Packet> &&packet) {
    ...
    if (phv->has_field(SSWITCH_PRIORITY_QUEUEING_RATE)) {
      uint64_t rate_Bps = phv->get_field(SSWITCH_PRIORITY_QUEUEING_RATE).get<uint64_t>();
      egress_buffers.set_rate(egress_port, SSWITCH_PRIORITY_QUEUEING_NB_QUEUES - 1 - priority, rate_Bps);
      BMLOG_DEBUG_PKT(*packet, "Setting queue {}, {} rate from P4 intrinsic_metadata.queue_rate", egress_port, priority);
    }

    if (phv->has_field(SSWITCH_PRIORITY_QUEUEING_DRR_QUANTUM)) {
      uint64_t quantum = phv->get_field(SSWITCH_PRIORITY_QUEUEING_DRR_QUANTUM).get<uint64_t>();
      egress_buffers.set_quantum(egress_port, SSWITCH_PRIORITY_QUEUEING_NB_QUEUES - 1 - priority, quantum);
      BMLOG_DEBUG_PKT(*packet, "Setting queue {}, {} quantum from P4 intrinsic_metadata.queue_quantum", egress_port, priority);
    }

    // since the DRR parameters cannot be set in P4Runtime,
    // set the priority 0 for queue-0
    // alternatively, it can be set the default quantum for queue-0
    if (egress_port == 0) {
      priority = 7;
    }
    ...
    egress_buffers.push_front(
        ...
        std::move(packet), packet->get_ingress_length());
#else
