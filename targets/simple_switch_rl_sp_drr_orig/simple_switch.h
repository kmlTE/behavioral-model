...
#define SSWITCH_PRIORITY_QUEUEING_RATE "intrinsic_metadata.queue_rate"
#define SSWITCH_PRIORITY_QUEUEING_DRR_QUANTUM "intrinsic_metadata.queue_quantum"
#endif

#define MAX_PORT_AND_WORKER_NUM 8u
...
class SimpleSwitch : public Switch {
 ...
 public:
  ...
  explicit SimpleSwitch(port_t max_port = MAX_PORT_AND_WORKER_NUM, bool enable_swap = false);
  ...
  int set_egress_priority_queue_rate(int port, size_t priority, const uint64_t rate_Bps);
  int set_egress_queue_rate(size_t port, const uint64_t rate_Bps);
  int set_all_egress_queue_rates(const uint64_t rate_Bps);

  int set_egress_priority_queue_quantum(int port, size_t priority,  const uint64_t quantum);
 ...
 private:
  static constexpr size_t nb_egress_threads = MAX_PORT_AND_WORKER_NUM;
...
#ifdef SSWITCH_PRIORITY_QUEUEING_ON
  bm::QueueingLogic_RLSP_DRR_ORIG<std::unique_ptr<Packet>, EgressThreadMapper>
#else
