...
class SimpleSwitchAPI(runtime_CLI.RuntimeAPI):
    ...
    def do_set_queue_rate(self, line):
        "Set rate of one / all egress queue(s): set_queue_rate <rate_Bps> [<egress_port>] [<priority>]"
        args = line.split()
    ...
    @runtime_CLI.handle_bad_input
    def do_set_queue_quantum(self, line):
        "Set quantum of one egress queue for DRR: set_queue_quantum <quantum> <egress_port> <priority>"
        args = line.split()
        ...
        try:
            ...
            if len(args) > 2:
                port = int(args[1])
                priority = int(args[2])
                self.sswitch_client.set_egress_priority_queue_quantum(port, priority, quantum)
            else:
                print "Some arguments are missing: quantum, port or priority value"
        except ValueError:
            print "Invalid quantum, port or priority value"
