...
class SimpleSwitchHandler : virtual public SimpleSwitchIf {
 public:
  ...
  int32_t set_all_egress_queue_rates(const int64_t rate_Bps) {
    bm::Logger::get()->trace("set_all_egress_queue_rates");
    return switch_->set_all_egress_queue_rates(static_cast<uint64_t>(rate_Bps));
  }

  int32_t set_egress_priority_queue_quantum(const int32_t port_num,
                                            const int32_t priority,
                                            const int64_t quantum) {
    bm::Logger::get()->trace("set_egress_priority_queue_quantum");
    return switch_->set_egress_priority_queue_quantum(port_num,
                                                      priority,
                                                      static_cast<uint64_t>(quantum));
  }
