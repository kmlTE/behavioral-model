...
SimpleSwitch::enqueue(port_t egress_port, std::unique_ptr<Packet> &&packet) {
    ...
    if (phv->has_field(SSWITCH_PRIORITY_QUEUEING_RATE)) {
      ...
      BMLOG_DEBUG_PKT(*packet, "Setting queue rate from P4 intrinsic_metadata.queue_rate");
    }
    ...
    if (phv->has_field(SSWITCH_PRIORITY_QUEUEING_DRR_QUANTUM)) {
      ...
      BMLOG_DEBUG_PKT(*packet, "Setting queue quantum from P4 intrinsic_metadata.queue_quantum");
    }
